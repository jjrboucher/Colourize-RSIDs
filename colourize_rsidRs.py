...
def apply_colour(rsids, excludedrsids):
    ...
    for rsid in rsids.keys():
        r_count_str = str(r_count)
        if r_count_str in colour_options:  # if a key exists for that #.

            rsids[rsid] = RGBColor(colour_options[r_count_str][1][0], colour_options[r_count_str][1][1],
                                   colour_options[r_count_str][1][2])  # assign a colour to this RSID
            logger.info(f'rsid {rsid} assigned colour {colour_options[r_count_str][1]} ({colour_options[r_count_str][0]})')
        r_count += 1
